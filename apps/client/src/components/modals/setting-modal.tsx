...
import { CentralModal, useCentralModal, registerModal } from "@/components/central-modal";

// Define the modal component
const SettingModal = () => {
  const modal = useCentralModal("setting-modal");
  const { t } = useTranslation();
  const [activeTab, setActiveTab] = useState("subspace");
  const [activeSubspaceId, setActiveSubspaceId] = useState<string | undefined>();

  // Get arguments passed to the modal
  const { tab, subspaceId } = modal.args || {};

  // Update state when modal is shown with new args
  useEffect(() => {
  ...
  return (
    <CentralModal id="setting-modal" className="h-5/6 max-h-[800px] max-w-6xl pb-0">
      <div className="sr-only">Setting Dialog</div>
...
// Register the modal
registerModal("setting-modal", SettingModal);
